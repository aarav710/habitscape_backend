...
func (pq *PostQuery) QueryHabit() *HabitQuery {
	query := &HabitQuery{config: pq.config}
...
func (pq *PostQuery) WithHabit(opts ...func(*HabitQuery)) *PostQuery {
	query := &HabitQuery{config: pq.config}
...
func (pq *PostQuery) sqlAll(ctx context.Context) ([]*Post, error) {
	...
	if pq.withHabit != nil || pq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withHabit; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Post)
		for i := range nodes {
			if nodes[i].habit_posts == nil {
				continue
			}
			fk := *nodes[i].habit_posts
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "habit_posts" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withLikes; query != nil {
		...
		for _, n := range neighbors {
			fk := n.post_likes
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "post_likes" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "post_likes" returned %v for node %v`, *fk, n.ID)
			}
