...
// QueryHabits chains the current query on the "habits" edge.
func (uq *UserQuery) QueryHabits() *HabitQuery {
	query := &HabitQuery{config: uq.config}
...
func (uq *UserQuery) QueryCompletions() *CompletionQuery {
	query := &CompletionQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithHabits tells the query-builder to eager-load the nodes that are connected to
// the "habits" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithHabits(opts ...func(*HabitQuery)) *UserQuery {
	query := &HabitQuery{config: uq.config}
...
func (uq *UserQuery) WithCompletions(opts ...func(*CompletionQuery)) *UserQuery {
	query := &CompletionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withCompletions = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withHabits; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Habits = []*Habit{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.HabitsPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "habits": %w`, err)
		}
		query.Where(habit.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "habits" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Habits = append(nodes[i].Edges.Habits, n)
			}
		}
	}
	...
	if query := uq.withAdmins; query != nil {
		...
		query.Where(predicate.Admin(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AdminsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_admins
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_admins" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_admins" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withCompletions; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Completion(func(s *sql.Selector) {
			s.Where(sql.InValues(user.CompletionsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_completions
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_completions" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_completions" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withLikes; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_likes
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_likes" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_likes" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withInvitations; query != nil {
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_invitations" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_invitations" returned %v for node %v`, *fk, n.ID)
			}
