...
func (aq *AdminQuery) QueryHabit() *HabitQuery {
	query := &HabitQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AdminQuery) QueryInvitations() *InvitationQuery {
	query := &InvitationQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AdminQuery) WithHabit(opts ...func(*HabitQuery)) *AdminQuery {
	query := &HabitQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AdminQuery) WithInvitations(opts ...func(*InvitationQuery)) *AdminQuery {
	query := &InvitationQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AdminQuery) sqlAll(ctx context.Context) ([]*Admin, error) {
	...
	if aq.withUser != nil || aq.withHabit != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withUser; query != nil {
		...
		for i := range nodes {
			if nodes[i].user_admins == nil {
				continue
			}
			fk := *nodes[i].user_admins
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_admins" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := aq.withHabit; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Admin)
		for i := range nodes {
			if nodes[i].habit_admins == nil {
				continue
			}
			fk := *nodes[i].habit_admins
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "habit_admins" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := aq.withInvitations; query != nil {
		...
		query.Where(predicate.Invitation(func(s *sql.Selector) {
			s.Where(sql.InValues(admin.InvitationsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.admin_invitations
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "admin_invitations" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "admin_invitations" returned %v for node %v`, *fk, n.ID)
			}
